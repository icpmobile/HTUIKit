...
-(void)titleClick{
    if (self.onClickTitleButton) {
        self.onClickTitleButton();
    }
}
...
- (void)wr_setLeftButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted title:(NSString *)title titleColor:(UIColor *)titleColor {
    ...
    self.leftButton.titleLabel.font = [UIFont systemFontOfSize:kWRDefaultTitleSize-2];
...
- (void)wr_setRightButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted title:(NSString *)title titleColor:(UIColor *)titleColor {
    ...
    self.rightButton.titleLabel.font = [UIFont systemFontOfSize:kWRDefaultTitleSize-2];
}
...
-(UILabel *)titleLable {
    if (!_titleLable) {
        ...
        [_titleLable addGestureRecognizer:labelTapGestureRecognizer];
        _titleLable.userInteractionEnabled = YES; // 可以理解为设置label可被点击_titleLable
...
-(int)navBarHeight{
   return [WRCustomNavigationBar navBarBottom];
}
@end
