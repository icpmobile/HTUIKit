...
static const CGFloat kTopSpaces = 10.f;
static const CGFloat kNumberMarkWidth = 0.f;
static const CGFloat kNumberMarkHeight = 0.f;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.colorArray = @[(id)[[self colorWithHex:0xFF6347] CGColor],
...
- (void)setNUmberMarkLayer { // 提示文字设置渐变色
    ...
    numberGradientLayer.frame = CGRectMake(0, kTopSpaces, self.frame.size.width, kNumberMarkHeight);
...
- (void)getGradientLayer { // 进度条设置渐变色
    ...
    bgLayer.frame = CGRectMake(kNumberMarkWidth / 2,  kTopSpaces, self.frame.size.width - kNumberMarkWidth / 2, kProcessHeight);
    bgLayer.backgroundColor = [self colorWithHex:0xF5F5F5].CGColor;
    bgLayer.masksToBounds = YES;
    bgLayer.cornerRadius = kProcessHeight / 2;
    [self.layer addSublayer:bgLayer];
    
    self.maskLayer = [CALayer layer];
    self.maskLayer.frame = CGRectMake(0, 0, (self.frame.size.width - kNumberMarkWidth / 2) * self.percent / 100.f, kProcessHeight);
    self.maskLayer.borderWidth = self.frame.size.height / 2;
    
    self.gradientLayer =  [CAGradientLayer layer];
    self.gradientLayer.frame = CGRectMake(kNumberMarkWidth / 2, kTopSpaces, self.frame.size.width - kNumberMarkWidth / 2, kProcessHeight);
...
- (void)circleAnimation { // 进度条动画
    ...
    self.maskLayer.frame = CGRectMake(0, 0, (self.frame.size.width - kNumberMarkWidth / 2) * _percent / 100.f, kProcessHeight);
