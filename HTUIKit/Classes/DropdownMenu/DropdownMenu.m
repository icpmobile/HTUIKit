...
- (void)showDropdownMenu:(NSInteger)index {
    ...
    if (index < _menuItems.count) {
        ...
        frame.size.height = [UIScreen mainScreen].bounds.size.height - m_frame.origin.y;
...
- (void)selectedFirstValues:(NSArray *)values withTitle:(NSString *)title {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(dropdownSelectedLeftIndex:RightIndex:TopIndex:)]) {
       // [_delegate performSelector:@selector(dropdownSelectedLeftIndex:RightIndex:) withObject:values[0] withObject:values[1]];
        
        [_delegate dropdownSelectedLeftIndex:[values[0] integerValue] RightIndex:[values[1] integerValue] TopIndex:[values[2] integerValue]];
...
- (DropdownButton *)dropdownButton {
    ...
    if (!_dropdownButton) {
        ...
        _dropdownButton = [[DropdownButton alloc] initWithFrame:frame WithDropdownButtonTitles:_titles ButtonCount:_titles.count];
...
- (UIView *)mask {
    ...
    if (!_mask) {
        CGFloat maskHeight = [UIScreen mainScreen].bounds.size.height - m_frame.origin.y;
